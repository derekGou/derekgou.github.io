...
function setup(){
    for (var y = 0; y<$(window).innerHeight()/size[1]; y++) {
        var blinks1 = []
...
function fill(x, y){
    num = (x+y)/($(window).innerHeight()/size[1]+($(window).width()/size[0]));
...
function blink(){
    if (phase==0)for (var y = 0; y<$(window).innerHeight()/size[1]; y++) {
        for (var x = 0; x<($(window).width()/size[0]); x++) {
            ...
            if ((rand<0.00005 || blinks[y][x]!=0) && bool==false){
                ...
                if (x>0){
                    ...
                    if ((y+1)<$(window).innerHeight()/size[1]){
                        fill(x-1, y+1)
                        fill(x, y+1)
                    }
                }
